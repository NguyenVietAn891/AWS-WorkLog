...
#### 5.2.1. Lựa chọn mô hình Client–Server: ReactJS/Electron và AWS Serverless
...
#### 5.2.2. Hạn chế và rào cản của kiến trúc
...
* **Debug phân tán:** Logic nằm rải rác trên các hàm Lambda (sơ đồ ghi nhận **43 fn**) cùng các trigger bất đồng bộ (Streams, S3, Cron) — theo dõi end-to-end đòi hỏi CloudWatch Logs được cấu hình đúng.
...
#### 5.2.3. Sơ đồ kiến trúc tổng thể
...
![Sơ đồ kiến trúc AWS](https://res.cloudinary.com/dakqspssm/image/upload/v1784649976/Uchimi_StudyGamification-Uchimi_Architecture.drawio_wdyyor.png)
...
3. **Invoke:** HTTP API invoke **Lambda (43 fn)** để xử lý business logic.
...
#### 5.2.4. Các dịch vụ AWS cốt lõi

Dự án sử dụng Serverless Framework (Node.js 20.x) để triển khai tự động cấu hình hạ tầng. Theo sơ đồ kiến trúc, lớp compute gồm **Lambda (43 fn)**; kèm theo **25 HTTP routes**, **2 EventBridge schedules** (cron ×2), **3 S3 triggers**, **1 DynamoDB Stream trigger** và **1 Cognito PostConfirmation trigger**.

| Dịch vụ | Mục đích sử dụng | Chi tiết cấu hình |
| :--- | :--- | :--- |
| **AWS Lambda** | Xử lý toàn bộ Business Logic | Nhóm API & COMPUTE trên sơ đồ: **43 fn**, tổ chức theo 10 module (User, Session, Gacha, Minigame, Quest, Shop, Social, Upload, Currency, Sync). |
...
**Amazon CloudFront** trên AWS Console (region **ap-southeast-1**):
...
**Amazon Bedrock (Account A — account dự án chính)** — cho phép `sts:AssumeRole` sang Account B:

![AssumeCrossAccountBedrockRole](https://res.cloudinary.com/dakqspssm/image/upload/v1784655482/bedrockchinh_mjqjus.jpg)

**Amazon Bedrock** — role cross-account và policy invoke:
...
**Amazon Bedrock (Account B)** — chỉ cấp quyền gọi 2 model `nova-micro-v1:0` và `nova-lite-v1:0`:
...
#### 5.2.6. Luồng giao tiếp giữa các thành phần
...
#### 5.2.7. Cấu hình bảo mật và Quyền truy cập (IAM Permissions)
...
#### 5.2.8. Quy trình thiết lập và triển khai hạ tầng
...
#### 5.2.9. Phân tách module Lambda và cơ chế kích hoạt
